...
| `write-plan` | Generate implementation plans as executable contracts with contract profiles (delivery/refactor/migration/exploration), checkpoints, verification, stop conditions, and handoff guidance. Includes validator script and tests / 将实施计划生成为可执行契约：contract profile、checkpoints、verification、stop conditions 和 handoff，附带校验脚本与测试 | Original | — |
