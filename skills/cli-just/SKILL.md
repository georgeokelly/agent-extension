...
name: cli-just
description: >-
  Expert guidance for Just, a command runner with syntax inspired by make.
  Use when the user asks to "create a justfile", "write just recipes",
  "configure just settings", "add just modules", "use just attributes",
  "set up task automation", mentions justfile, just command runner, or
  task automation with just.

# Spec (optional)
license: MIT
compatibility: Cross-tool (Cursor, Claude Code, Codex).
metadata:
  author: georgel
  version: "1.0"
  upstream: https://github.com/PaulRBerg/agent-skills/tree/main/skills/cli-just
  upstream_author: Paul Razvan Berg
...
# Spec (claude-only)
when_to_use: >-
  Use when creating justfiles, writing recipes with attributes/dependencies/
  parameters, configuring settings for shell/modules/imports, using built-in
  terminal-formatting constants, or implementing check/write automation
  patterns with Just.
argument-hint: "[target justfile or recipe context]"
...
> **Attribution**: Adapted from [PaulRBerg/agent-skills/cli-just](https://github.com/PaulRBerg/agent-skills/tree/main/skills/cli-just) by Paul Razvan Berg, licensed under [MIT](./LICENSE). Frontmatter was rewritten to match the agentskills.io specification used across this repository; the body content is preserved verbatim.
